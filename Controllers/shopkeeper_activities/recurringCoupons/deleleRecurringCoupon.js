...
const DeleteRecurringCoupon = async (req, res) => {
  try {
    ...
    const coupon = await RecurringCouponModel.findOne({ _id: couponId, shopkeeper: shopkeeper._id });
    if (!coupon) {
      return res.status(404).json({ error: 'Coupon not found for this shopkeeper' });
    }

    await RecurringCouponModel.findByIdAndDelete(couponId);
...
export default DeleteRecurringCoupon;
