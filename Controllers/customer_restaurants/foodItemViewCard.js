...
const addFoodItemsViewCard = async (req, res) => {
  try {
    ...
    for (const [foodId, item] of Object.entries(data)) {
      ...
      // Try FoodSubCategory first
      let subFood = await FoodSubCategory.findById(foodId);
      let isAddOn = false;

      // If not found, try AddOnModel
      if (!subFood) {
        subFood = await AddOnModel.findById(foodId);
        if (!subFood) continue; // skip invalid id
        isAddOn = true;
      }
      ...
      if (existingCartItems.length > 0) {
        const existingRestaurantId = existingCartItems[0].restuarantId.toString();
        const newRest = subFood.restaurant.toString();

        if (existingRestaurantId !== newRest) {
          const restaurantName = await RestaurantOwnerModel.findById(existingRestaurantId);
      ...
      let cartItem;
      if (isAddOn) {
        cartItem = await FoodItemsModel.findOne({
          userId: customer._id,
          addOnId: subFood._id
        });
      } else {
